...
#define EXTERNAL_SX127x_BOARD_ON_ESP32DEV
...
int16_t lwActivate() {
  ...
  //  if (!store.clear()) {
  //    Serial.println(F("Error clearing persistent namespace 'radiolib'"));
  //  }
  ...
  if (store.isKey("nonces")) {
    ...
    debug(state != RADIOLIB_ERR_NONE, F("Restoring nonces buffer failed"),
          state, false);
    ...
    debug((state != RADIOLIB_ERR_NONE) && (bootCount > 1),
          F("Restoring session buffer failed"), state, false);
    ...
    if (state == RADIOLIB_ERR_NONE) {
      ...
      debug((state != RADIOLIB_LORAWAN_SESSION_RESTORED),
            F("Failed to activate restored session"), state, true);
  ...
  while (state != RADIOLIB_LORAWAN_NEW_SESSION) { // Original code
    ...
    if (state != RADIOLIB_LORAWAN_NEW_SESSION) {
      ...
      uint32_t sleepForSeconds =
          min((bootCountSinceUnsuccessfulJoin++ + 1UL) * 60UL, 3UL * 60UL);
...
void setup() {
  ...
  if (fCntUp == 1) {
    ...
    state = node.sendReceive(uplinkPayload, sizeof(uplinkPayload) - 1, fPort,
                             downlinkPayload, &downlinkSize, true,
                             &uplinkDetails, &downlinkDetails);
  } else {
    state = node.sendReceive(uplinkPayload, sizeof(uplinkPayload) - 1, fPort,
                             downlinkPayload, &downlinkSize, false,
                             &uplinkDetails, &downlinkDetails);
  }
  ...
  //  condition
  debug((state < RADIOLIB_ERR_NONE) && (state != RADIOLIB_ERR_NONE),
        F("Error in sendReceive"), state, false); // This is correct
